  ...
  return {
    ...
    'steps': [
      ...
      },
      {
        ...
        'commands': [
          'echo $env:PASSWORD | docker login --username $env:USERNAME --password-stdin',
          'docker build --pull -f docker/Dockerfile.windows.%s -t dheerajkhardwal/drone-maven:${DRONE_TAG##v}-windows-%s-amd64 .' % (version, version),
          'docker run --rm dheerajkhardwal/drone-maven:${DRONE_TAG##v}-windows-%s-amd64 --help' % version,
          'docker push dheerajkhardwal/drone-maven:${DRONE_TAG##v}-windows-%s-amd64' % version,
        ],
        'when': {
          'ref': [
            'refs/tags/**',
